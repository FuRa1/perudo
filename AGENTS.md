# AGENTS.md — Perudo "Pirates of the Caribbean"
...
## 0. START HERE — ORIENT BEFORE YOU BUILD

**Every session starts cold.** You have no memory of any earlier session, even if you were "just working on this" — nothing carries over. This file is your only starting context, and it describes the _intended_ state, not necessarily _today's actual_ state. Do the following, in this exact order, before writing or changing a single line:

1. **Read this file in full first.** Do not start implementing from this read alone — it's orientation, not a task.
2. **Check the real state against the code, not just this document's claims.** Run `git status --short` and skim recent `git log`. Section 1 (Phase Status) below says Phases 1–6 are complete — that claim can drift out of date as work happens outside this file being updated, so spot-check it against the actual code and tests rather than taking it purely on faith. Do not redo work that already exists; do not assume something is missing just because this file doesn't mention it.
3. **Locate before you invent.** For whatever the actual task turns out to be, search the existing code (`rg`) for where that behavior already lives before writing a parallel implementation. `/shared` holds domain logic reused by both client and server — check there first, always.
4. **Only then start the task** — and use this project's OpenCode skills for _how_ to work, not just _what_ to build:
   - `/tdd <task>` — implement test-first: failing test before implementation code, every time.
   - `/verify` — the lint/typecheck/test/build gate. Run it before calling anything done, not just when it seems convenient.
   - `/e2e` — the Playwright suite, for changes touching transport, gameplay flow, reconnect, or timers.
   - `/stack-check <topic>` — check current docs before using an unfamiliar Angular/Ionic/NestJS/Socket.io API rather than trusting memory on a fast-moving stack.
   - `@reviewer` — an independent, read-only second pass before handing off anything non-trivial.

Then, the ongoing rules for every task:
...
- Every phase ends in a **working, committed, lint-clean, type-clean state** (see 4.4) — confirmed via `/verify`, not assumed.
...
- If this file and the actual code disagree, trust the code, and say so — don't silently pick one and move on.
