...
**Project skills (Claude Code and Codex):** read
`.agents/skills/perudo-workflow/SKILL.md` and use relevant skills automatically
for the current task. `.claude/skills` entrypoints load shared instructions from
`.agents/skills`; see `SKILLS.md` for sources and usage. Current user/host
instructions and project requirements take priority over downloaded guidance.
For UI work, `designs/perudo-mobile-consolidated.dc.html` is the canonical
reference (see `ISSUES.md` and the later addenda), superseding Phase 4's history.
...
- **Addendum (2026-09-06, outside the original 6-phase roadmap) — swappable board dashboards
  (Default/Clockwise/Linear).** Built per `plans/board-dashboard-switching.md` (full decision
  record, ring math, and phase breakdown there — not restated here), an **experimental** feature
  exploring alternative live-table arrangements from `designs/perudo-spotlight-gallery.dc.html`
  ("TURN-ORDER GALLERY · CONCEPT", explicitly not part of the canonical
  `perudo-mobile-consolidated.dc.html`). `features/table/table.ts` — previously one monolithic
  component — was split into `features/board/board-shell/` (header/switcher/bid-rail/
  `RoundLossModal`, shell content rendered identically across all three layouts) plus a `@switch`
  over three dashboards: `DefaultDashboard` (today's board, reproduced verbatim — zero behavior
  change, verified by the full `lobby-flow`/`bidding-flow`/`reconnect-timer-flow` e2e suite passing
  **unmodified**), `LinearDashboard` (Variant A "Below deck" — a spotlight card for whoever's
  bidding + a horizontal turn-ordered rail of everyone else, built from `ArcSeat`), and
  `ClockwiseDashboard` (Variant B "Round the table" — waiting players on an ellipse ring around the
  spotlight; ring radius, seat size, and tail-dimming are continuous functions of opponent count,
  not breakpointed — `features/board/seat-geometry.ts`). A new `LayoutStore` (`@ngrx/signals@21.1.1`
  — pinned, not `@latest`, which needs Angular 22 — this repo's first NgRx package, signals-native
  so no RxJS idiom enters an otherwise 100%-signals codebase) drives a fixed three-way cycle via one
  header pill (`↻ <next layout>`), persisted to `localStorage['perudo:boardLayout']`. Both new
  layouts fall back to `DefaultDashboard`'s own arc outside BIDDING (START_ROLL/ROUND_ROLLING,
  which the gallery never designed for them), and Phase 4 added a scale+fade entrance for the
  spotlight/hero card on a turn change plus a CSS position transition for ring seats (the "seats
  move between slots" model), both gated behind `prefers-reduced-motion` (same precedent as
  `ui/loader/loader.scss`/`ui/turn-timer/turn-timer.scss`). New verification tooling:
  `client/e2e/layout-render.mjs` (screenshots a layout at 2/4/8/12 players via the fixture loader +
  a seeded `localStorage` key, since nothing else can reach a non-default layout without tapping
  the switcher) and `client/e2e/reduced-motion-check.mjs` (asserts the computed `animation`/
  `transition` actually neutralize under reduced motion, not just that a class toggles). **Not
  done:** Phase 0 (a design-parity pass shrinking the reference's hero card) never ran, which is
  the likely fix for the one known open visual issue — at the smallest tables the ring's two seats
  flanking the reserved bottom gap graze the (necessarily taller, since it reuses `BidMarker`
  rather than the reference's bespoke compact layout) hero card's corner by a few pixels, visible in
  `client/e2e/.screenshots/layout-render/clockwise-4p.png`. Full status, verification detail, and a
  list of other open judgment calls: `plans/board-dashboard-switching-status.md`. Also logged in
  `ISSUES.md`. **Verified per phase:** `npm run lint`, `typecheck`, `build` clean throughout;
  317 client tests (65 shared + 37 server unchanged — no server/shared code touched by this
  addendum); the full `npm run e2e` chain passes unmodified after every phase.
